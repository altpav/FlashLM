...
parser.add_argument("--small", action="store_true",
                    help="Small config: d=192, 6 blocks, ~5M params")
parser.add_argument("--large", action="store_true",
                    help="Large config: d=384, 8 blocks, ~17M params")
...
if args.small:
    ...
    CONFIG_NAME = "teeny-delta-s"
else:
    DIM = 384
    N_BLOCKS = 8
    GLU_HIDDEN = 1024
    SEQ_LEN = 1024  # Increased from 512
    NUM_HEADS = 6
    BATCH_SIZE = args.batch or max(4, min(64, NUM_CORES * 2))
    GRAD_ACCUM = 2
    LR_PEAK = args.lr or 3e-3
    CONFIG_NAME = "teeny-delta-l"
...
class ShortConvolution(nn.Module):
    ...
    def forward(self, x):
        ...
        x_padded = F.pad(x.transpose(1, 2), (pad, 0), value=0)  # (B, D, T+pad)
        # Depthwise conv (groups=D)
        out = F.conv1d(x_padded, self.weight, bias=self.bias, groups=D, padding=0)
        return out[:, :, :T].transpose(1, 2)  # (B, T, D)

class GatedDeltaNet(nn.Module):
    """
    Gated Delta Network - Linear attention with gating and delta rule.
    """
    def __init__(self, dim, num_heads, expand_k=0.75, expand_v=1.5, conv_size=4):
        super().__init__()
        self.dim = dim
        ...
        # Projections
        self.q_proj = BitLinear(dim, self.key_dim)
        self.k_proj = BitLinear(dim, self.key_dim)
        self.v_proj = BitLinear(dim, self.value_dim)
        self.g_proj = BitLinear(dim, self.value_dim)  # output gate
        
        # Short conv
        self.q_conv = ShortConvolution(self.key_dim, conv_size)
        self.k_conv = ShortConvolution(self.key_dim, conv_size)
        self.v_conv = ShortConvolution(self.value_dim, conv_size)
        
        # Gates: per head
        self.gk_proj = BitLinear(dim, num_heads)  # decay gate
        self.beta_proj = BitLinear(dim, num_heads)  # update gate
        
        # Output
        self.o_proj = BitLinear(self.value_dim, dim)
        
    def forward(self, x):
        B, T, D = x.shape
        
        # Project and apply short conv
        q = F.silu(self.q_conv(self.q_proj(x)))  # (B, T, key_dim)
        k = F.silu(self.k_conv(self.k_proj(x)))  # (B, T, key_dim)
        v = F.silu(self.v_conv(self.v_proj(x)))  # (B, T, value_dim)
        
        # Gates
        gk = F.logsigmoid(self.gk_proj(x)) / 16  # (B, T, num_heads), decay
        beta = torch.sigmoid(self.beta_proj(x))   # (B, T, num_heads), update
        
        # Reshape for heads: (B, T, H, head_dim) -> (B, H, T, head_dim)
        q = q.view(B, T, self.num_heads, self.head_qk_dim).transpose(1, 2)
        k = k.view(B, T, self.num_heads, self.head_qk_dim).transpose(1, 2)
        ...
        # Reshape gates: (B, T, H) -> (B, H, T, 1)
        gk = gk.transpose(1, 2).unsqueeze(-1)  # (B, H, T, 1)
        beta = beta.transpose(1, 2).unsqueeze(-1)  # (B, H, T, 1)
        
        # Gated delta rule - recurrent computation
        # State S: (B, H, head_qk_dim, head_v_dim)
        S = torch.zeros(B, self.num_heads, self.head_qk_dim, self.head_v_dim,
                       device=x.device, dtype=x.dtype)
        outputs = []
        
        for t in range(T):
            # Current timestep
            q_t = q[:, :, t, :]  # (B, H, head_qk_dim)
            k_t = k[:, :, t, :]  # (B, H, head_qk_dim)
            v_t = v[:, :, t, :]  # (B, H, head_v_dim)
            gk_t = gk[:, :, t, :]  # (B, H, 1)
            beta_t = beta[:, :, t, :]  # (B, H, 1)
            
            # Compute S @ k_t: (B, H, head_v_dim)
            Sk = torch.einsum('bhdk,bhd->bhk', S, k_t)
            
            # Delta: v_t - Sk
            delta = v_t - Sk  # (B, H, head_v_dim)
            
            # Update state: S_t = decay * S_{t-1} + beta * outer(k_t, delta)
            # outer(k_t, delta): (B, H, head_qk_dim, head_v_dim)
            outer = torch.einsum('bhd,bhk->bhdk', k_t, delta)
            S = torch.exp(gk_t).unsqueeze(-1) * S + beta_t.unsqueeze(-1) * outer
            
            # Output: o_t = q_t @ S_t
            o_t = torch.einsum('bhd,bhdk->bhk', q_t, S)  # (B, H, head_v_dim)
            outputs.append(o_t)
        
        o = torch.stack(outputs, dim=2)
        
        # Reshape: (B, H, T, head_v_dim) -> (B, T, value_dim)
        o = o.transpose(1, 2).contiguous().view(B, T, self.value_dim)
        
        g = torch.sigmoid(self.g_proj(x))
        return self.o_proj(o * g)
...
class ImliBlock(nn.Module):
    def __init__(self, dim, num_heads, glu_hidden, expand_k=0.75, expand_v=1.5, conv_size=4):
        super().__init__()
        self.norm1 = RMSNorm(dim)
        self.mixer = GatedDeltaNet(dim, num_heads, expand_k, expand_v, conv_size)
...
class TeenyLM(nn.Module):
    def __init__(self, vocab_size=VOCAB_SIZE, dim=DIM, n_blocks=N_BLOCKS,
                 num_heads=NUM_HEADS, glu_hidden=GLU_HIDDEN, 
                 expand_k=EXPAND_K, expand_v=EXPAND_V, conv_size=CONV_SIZE):
        super().__init__()
        self.vocab_size = vocab_size
        self.dim = dim
        self.embedding = nn.Embedding(vocab_size, dim)
        self.blocks = nn.ModuleList([
            ImliBlock(dim, num_heads, glu_hidden, expand_k, expand_v, conv_size)
...
def train():
    ...
    logger.info(f"")
    logger.info(f"{'='*60}")
    logger.info(f"  TeenyLM Delta — {CONFIG_NAME}")
    logger.info(f"{'='*60}")
    logger.info(f"  CPU cores: {NUM_CORES}")
    logger.info(f"  RAM: {TOTAL_RAM_GB:.1f} GB")
    logger.info(f"  PyTorch threads: {torch.get_num_threads()}")
    logger.info(f"  Config: d={DIM}, heads={NUM_HEADS}, blocks={N_BLOCKS}, glu={GLU_HIDDEN}")
    ...
    if args.resume:
        ...
        if "model_state_dict" in ckpt:
            ...
            logger.info(f"  Resumed at step {global_step}, epoch {start_epoch}, best val loss {best_val_loss:.4f}")
        else:
            model.load_state_dict(ckpt, strict=False)
            logger.info(f"  Loaded weights only")
        ...
        for batch_idx, (x, y) in enumerate(train_loader):
            ...
            if (batch_idx + 1) % GRAD_ACCUM == 0:
                ...
                if now - last_log_time >= log_interval:
                    ...
                    eta = f" | ETA {(TIME_LIMIT - elapsed)/60:.0f}min" if TIME_LIMIT else ""
                    logger.info(f"  Step {global_step:>6d} | loss {accum_loss:.4f} | lr {lr:.6f} | {tok_s:.0f} tok/s | {tokens_seen/1e6:.1f}M tokens{eta}")
                    last_log_time = now
                accum_loss = 0.0

                if global_step % EVAL_EVERY == 0:
                    val_loss = evaluate(model, val_loader)
                    logger.info(f"\n  >>> Val loss: {val_loss:.4f} (best: {best_val_loss:.4f})\n")
                ...
                if global_step % SAVE_EVERY == 0:
                    ...
                    torch.save({
                        ...
                        "config": {
                            ...
                            "n_blocks": N_BLOCKS, "num_heads": NUM_HEADS,
